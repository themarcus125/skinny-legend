...
test('the feed groups both members entries under a day heading', async ({ page }) => {
  await signInWeb(page, ha);
  // The group log lives on Trang chủ since SKI-134; `/feed` only redirects here.
...
test('the map shows a pin per located entry', async ({ page }) => {
  ...
  // The group log lives on Trang chủ since SKI-134; `/feed` only redirects here.
  await page.goto(`${WEB_URL}/`);
  await page.getByRole('button', { name: /trên bản đồ/ }).first().click();
  await page.waitForURL(/\/feed\/map\?entry=/);
